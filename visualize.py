...
class Animation:
  def __init__(self, map, schedule):
    ...
    self.obstacleSchedule = self.schedule['obstacles']
    self.obstacles = {}
    ...
    self.patches.append(Rectangle((xmin, ymin), xmax - xmin, ymax - ymin, facecolor='none', edgecolor='grey'))
    for o in map["map"]["obstacles"]:
      x, y = o[0], o[1]
      self.obstacles[str(x) + str(y)] = Rectangle((x - 0.5, y - 0.5), 1, 1, facecolor='grey', edgecolor='grey')
      self.patches.append(self.obstacles[str(x) + str(y)])
      self.artists.append(self.obstacles[str(x) + str(y)])
  ...
  def animate_func(self, i):
    # reset obstacle colors back to grey when animation restarts
    if i == 0:
      for time in self.obstacleSchedule:
        for obstacle in self.obstacleSchedule[time]:
          x, y = obstacle
          self.obstacles[str(x) + str(y)].set_color('grey')
    ...
    maxTime = max(self.obstacleSchedule)
    if int(i/10) < maxTime:
      for obstacle in self.obstacleSchedule[int(i / 10)]:
        x, y = obstacle
        self.obstacles[str(x) + str(y)].set_color('red')
