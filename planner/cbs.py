...
from planner.a_star import AStar
...
class Environment(object):
    ...
    def admissible_heuristic(self, state, agent_name):
        ...
        return 64 *  (fabs(state.location.x - goal.location.x) + fabs(state.location.y - goal.location.y))
...
class CBS(object):
    ...
    def generate_plan(self, solution):
        ...
        for agent, path in solution.items():
            if len(path) > 1:
                path_dict_list = {'t':path[1].time, 'x':path[1].location.x, 'y':path[1].location.y}
            else:
                path_dict_list = {'t':path[0].time, 'x':path[0].location.x, 'y':path[0].location.y}
