...
//combine
	...
	{
	    T[0] = RndEngine::GetSingleton()->UniformReal();
	    T[1] = RndEngine::GetSingleton()->UniformReal();
	}
	static inline void TransSampleUniform(double T[], const double bbox[])
	{
	    T[0] = RndEngine::GetSingleton()->UniformReal(bbox[0], bbox[1]);
	    T[1] = RndEngine::GetSingleton()->UniformReal(bbox[2], bbox[3]);
	}
	static inline double AngleSampleUniform(void)
	{
	    return RndEngine::GetSingleton()->UniformReal(-M_PI, M_PI);
	}
	static inline double AngleSampleUniform(const double bbox[])
	{
	    return RndEngine::GetSingleton()->UniformReal(bbox[0], bbox[1]);
	}
	static inline void RotSampleUniform(double R[])
	{
	    const double c = RndEngine::GetSingleton()->UniformReal(-1, 1);
	    
	    R[Rot_COS] = c;		
	    R[Rot_SIN] = sqrt(1 - c * c) * (RndEngine::GetSingleton()->UniformBoolean() ? 1:-1);
	}
	static inline void RotSampleUniform(double R[], const double bbox[])
	{
	    const double theta = RndEngine::GetSingleton()->UniformReal(bbox[0], bbox[1]);
	    R[Rot_COS] = cos(theta);		
	    R[Rot_SIN] = sin(theta);
	}
