 ...
 *  .
 */
	template <typename Key, typename LessCmp = std::less<Key> >
	int Contains(const Key p[], const Key key, const int start, const int end, LessCmp cmp = LessCmp())
	{
	    int s = start;
	    int e = end;
	    int m = 0;
		...
		if (cmp(p[m], key)) //p[m] < key
		    s = m + 1;
		else if (cmp(key, p[m])) //key < p[m]
		    e = m - 1;
		else
		    return m;
	    }
 ...
 */
	template <typename Key, typename LessCmp = std::less<Key> >
	int SortedPosition(const Key p[], const Key key, const int start, const int end, LessCmp cmp = LessCmp())
	{
	    int s = start;
	    int e = end;
	    int m = 0;
