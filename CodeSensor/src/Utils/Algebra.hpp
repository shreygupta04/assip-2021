...
namespace Antipatrea
{
	...
	static inline double PointsDistanceSquared(const int dim, const int n, const double pts[])
	{
	    double d = 0.0;
	    for(int i = 1; i < n; ++i)
		d += PointDistanceSquared(dim, &pts[(i-1) * dim], &pts[i * dim]);
	    return d;
	}

	static inline double PointsDistance(const int dim, const int n, const double pts[])
	{
	    return sqrt(PointsDistanceSquared(dim, n, pts));
	}
