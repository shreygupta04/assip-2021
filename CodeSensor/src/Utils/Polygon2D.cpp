...
namespace Antipatrea
{
    ...
    void Polygon2D::AddToTriMesh(TriMesh &tmesh)
    {
	tmesh.AddPolygon(*this);
    }
    ...
    void Polygon2D::OccupiedGridCells(const Grid &grid,   std::vector<Id> &cids)
    {
	const double *bbox = GetBoundingBox();
	...
	RndEngine::GetSingleton()->SamplePointInsideTriangle2D(&m_vertices[2 * m_triIndices[3 * t]],
    ...
    {
	const int n = GetNrTriangles();
	const double r = RndEngine::GetSingleton()->UniformReal(0, GetArea());
	double w = 0;
