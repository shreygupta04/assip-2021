...
{
    static inline bool SameReal(const double x, const double y)
    {
    ...
    template <typename Key> static inline void MinMax(const Key &key1, const Key &key2, Key &keyMin, Key &keyMax)
    {
	if(key1 < key2)
	{
	    keyMin = key1;
    ...
    template<typename Item> static inline void SwapItems(Item &val1, Item &val2)
    {
	auto tmp = val1;
	val1 = val2;
	val2 = tmp;
 ...
 */
	...
	for (int i = 0; i < r; i++)
	{
	    int j = RndEngine::GetSingleton()->UniformInteger(i, n - 1);
	    Item t = items[i];
	    items[i] = items[j];
	    items[j] = t;
	}
