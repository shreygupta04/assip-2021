...
//sample
	...
	{	
	    ...
	    do
	    {
		x = RndEngine::GetSingleton()->UniformReal(-1, 1);
		y = RndEngine::GetSingleton()->UniformReal(-1, 1);	
	    }
	    while((s1 = (x * x + y * y)) >= 1.0);
	    
	    do
	    {
		z = RndEngine::GetSingleton()->UniformReal(-1, 1);
		w = RndEngine::GetSingleton()->UniformReal(-1, 1);			
		s2= z * z + w * w;	
	    }
	    while(s2 >= 1.0 || s2 == 0.0);
	...
	void QuatSampleUniform_Shoemake(double Q[])
	{
	    ...
	    const double s     = RndEngine::GetSingleton()->UniformReal();
	    const double s1    = sqrt(1 - s);
	    const double s2    = sqrt(s);
	    const double a1    = twoPi * RndEngine::GetSingleton()->UniformReal();
	    const double a2    = twoPi * RndEngine::GetSingleton()->UniformReal();
