...
namespace Antipatrea
{
    ...
    void TriMesh::AddPolygon(Polygon2D & poly, const double z)
    {
    ...
    void TriMesh::AddCappedPyramid(Polygon2D & basePoly,
				   Polygon2D & topPoly,
				   const double zmin,
				   const double zmax)
    {
	...
	auto startBase = GetNrVertices();
	AddPolygon(basePoly, zmin);
	auto startTop = GetNrVertices();
	AddPolygon(topPoly, zmax);
	
	AddCapCapStripCCW(std::min(basePoly.GetNrVertices(), topPoly.GetNrVertices()), startBase, startTop,  closed);
    }

    void TriMesh::AddSkewedCylinderZ(const int n,
				     const double x1, const double y1, const double z1, const double r1,
	...
	poly.resize(2 * n);
	CircleAsPolygon2D(x1, y1, r1, n, &poly[0]);
	base.AddVertices(n, &poly[0]);
	
	CircleAsPolygon2D(x2, y2, r2, n, &poly[0]);
	top.AddVertices(n, &poly[0]);
	
	AddCappedPyramid(base, top,  z1, z2);	
    }
    
    void TriMesh::AddSkewedCylinderX(const int n,
				     const double x1, const double y1, const double z1, const double r1,
				     const double x2, const double y2, const double z2, const double r2)
    {
	double TR[Algebra3D::TransRot_NR_ENTRIES];
      	double T2[Algebra3D::Trans_NR_ENTRIES];       
	double TR2[Algebra3D::TransRot_NR_ENTRIES];
       
	const int offset = GetNrVertices();
	AddSkewedCylinderZ(n, x1, y1, z1, r1, x2, y2, z2, r2);

	TR[0] = x1;
	TR[1] = y1;
	TR[2] = z1;	
	Algebra3D::YAxisAngleAsRot(0.5 * M_PI, &TR[Algebra3D::Trans_NR_ENTRIES]);
	T2[0] = -x1;
	T2[1] = -y1;
	T2[2] = -z1;	
	Algebra3D::TransRotMultTransAsTransRot(TR, T2, TR2);
	
	ApplyTransRot(offset, GetNrVertices() - 1, TR2);
	//ApplyRot(offset, GetNrVertices() - 1, R);
	//ApplyTrans(offset, GetNrVertices() - 1, x1, y1, z1);
    }

    
    void TriMesh::AddSkewedCylinderY(const int n,
				     const double x1, const double y1, const double z1, const double r1,
				     const double x2, const double y2, const double z2, const double r2)
    {
	double TR[Algebra3D::TransRot_NR_ENTRIES];
	...
	TR[1] = y1;
	TR[2] = z1;	
	Algebra3D::XAxisAngleAsRot(-0.5 * M_PI, &TR[Algebra3D::Trans_NR_ENTRIES]);
	T2[0] = -x1;
	T2[1] = -y1;
	T2[2] = -z1;	
	Algebra3D::TransRotMultTransAsTransRot(TR, T2, TR2);
    ...
    void TriMesh::DrawColormap(void) 
    {
	const double *min = GetBoundingBoxMin();
	const double *max = GetBoundingBoxMax();
	const int     nf  = GetNrFaces();
	GMaterial     gmat;
	...
	for(int i = 0; i < nf; ++i)
	{
	    ...
	    for(int j = 0; j < fnv; ++j)
	    {
		glNormal3dv(GetVertexNormal(f->m_vids[j]));

		
//estuary: 
//		const double  zmax = std::min(0.0, max[2]);
//		const double  zmin = std::max(-30.0, min[2]);		

//river:
		const double  zmax = max[2];
		const double  zmin = min[2];		
		const double *v    = GetVertex(f->m_vids[j]);
		const double  zval = v[2];		
		const double  d    = (zval - zmin) / (zmax - zmin);
	
		//	if(v[2] >= zmax)
		//    gmat.SetDiffuse(0.4, 0.4, 0.4);
		//else
		{
		    gmat.SetDiffuse(Colormap::GetSingleton()->GetRed(d),
				    Colormap::GetSingleton()->GetGreen(d),
		    		    Colormap::GetSingleton()->GetBlue(d));
		}
...
//	GDraw3D();
	...
	const int n = GetNrVertices();
	for (int i = 0; i < n; ++i)
	    AddTexCoord(RndEngine::GetSingleton()->UniformReal(0, 1),
			RndEngine::GetSingleton()->UniformReal(0, 1));
    }
    ...
    void TriMesh::SamplePointInsideFace(const int fid, double p[])
    {
	const Face *f = GetFace(fid);
	const int   i = RndEngine::GetSingleton()->UniformInteger(0, f->m_vids.size() - 1);
	const int   j = (i + 1) % ((int) f->m_vids.size());

	const double *tA = f->m_centroid;
	const double *tB = GetVertex(f->m_vids[i]);
	const double *tC = GetVertex(f->m_vids[j]);
 	
	double a1 = RndEngine::GetSingleton()->UniformReal();
	double a2 = RndEngine::GetSingleton()->UniformReal();
	...
	p[2] = tA[2] + a1 * (tB[2] - tA[2]) + a2 * (tC[2] - tA[2]);
    }

    
    void TriMesh::GetFacesBelow(const double z, std::vector<int> & fids)
    {
	fids.clear();
	...
	for(int i = 0; i < n; ++i)
	{
	    ...
	    const int   fnv   = f->m_vids.size();
	    bool        below = true;
	    for(int j = 0; j < fnv && below; ++j)
		below = GetVertex(f->m_vids[j])[2] < z;
	    if(below)
		fids.push_back(i);
	}
    }
